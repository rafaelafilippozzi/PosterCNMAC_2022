...
function [index,iter_num]=AVTA_ASFW(Q_bar,GAMMA)
...
while epsilon>GAMMA*sqrt(max(eudis))
    ...
    while sum(candidate_set)>0
        ...
        [inorout,p_prime,alpha_coe,iter_val]=AwayStepFrankWolfeAlgorithmAVTA(this_data,p,epsilon, alpha_zero(:,this_index));
